...
def write_tf_messages_to_bag(tf_messages: list[TFMessage], stamps_ns: list[int],
    ...
    with rosbags.rosbag2.Writer(path) as writer:
        connection = writer.add_connection('/tf', 'tf2_msgs/msg/TFMessage')
        for stamp_ns, msg in zip(stamps_ns, tf_messages):
            serialized_msg = rosbags.serde.serialize_cdr(msg, 'tf2_msgs/msg/TFMessage')
