...
#ifndef ISAAC_COMMON__MESSAGING__MESSAGE_STREAM_SYNCHRONIZER_HPP_
#define ISAAC_COMMON__MESSAGING__MESSAGE_STREAM_SYNCHRONIZER_HPP_
...
public:
  ...
  explicit MessageStreamSynchronizer(
    int num_topics, int timestamp_delta_threshold_ns, int min_num_messages, int buffer_size);
  ...
  const std::vector<int64_t> & values, int64_t threshold)
{
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    if (values[i] <= min_value + threshold) {
      indices.push_back(i);
    }
  }
  return indices;
...
void MessageStreamSynchronizer<Message>::AddMessage(
  int idx, int64_t timestamp, const Message & message, bool trigger_callback)
{
...
template<typename Message>
auto MessageStreamSynchronizer<Message>::PeekNextMessage(int idx) const -> Message
{
  return message_buffers_[idx].Peek();
}

template<typename Message>
std::vector<int64_t> MessageStreamSynchronizer<Message>::PeekMessageTimestamps() const
{
...
template<typename Message>
void MessageStreamSynchronizer<Message>::RegisterCallback(CallbackFunction callback)
{
  callback_ = std::move(callback);
}

template<typename Message>
void MessageStreamSynchronizer<Message>::ClearBuffers()
{
  for (auto & buffer : message_buffers_) {
    buffer.ClearAll();
  }
}
