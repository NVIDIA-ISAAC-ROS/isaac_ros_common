...
ExampleMessage CreateMessage(int time_ns)
{
  ExampleMessage message;
  message.sec = time_ns / 1e9;
  message.nanosec = time_ns % static_cast<int>(1e9);
  return message;
}

template<typename Message>
void ExpectTimestamps(
  const std::vector<std::pair<int, Message>> & messages, const std::vector<int> & indices,
  const std::vector<int> & timestamps)
{
  ...
  for (size_t i = 0; i < timestamps.size(); ++i) {
    ...
    const int msg_stamp = messages[i].second.sec * 1e9 + messages[i].second.nanosec;
...
TEST(MessageStreamSynchronizerTests, EverythingTest) {
  ...
  MessageStreamSynchronizer<ExampleMessage> sync(
    kNumTopics, kTimestepDeltaThresholdNs, kNumTopics, kBufferSize);

  // Use a lambda to store the messages from the callback.
  std::vector<std::pair<int, ExampleMessage>> synced_msgs;
